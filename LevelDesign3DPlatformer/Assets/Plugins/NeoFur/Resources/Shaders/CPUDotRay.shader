Shader "NeoFurDebug/CPUDotRay"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4		RayColor;	//color of the velocity
			...
			GSInput	DotRayVS(VSInput v)
			{
				...
				float3	RayVec	=lerp(ray, pos + ray, isDelta);

				float4	pos0	=float4(pos, 1);
				float4	pos1	=float4(RayVec, 1);
